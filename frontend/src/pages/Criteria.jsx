...
export const Criteria = () => {
  ...
  const columns = [
    ...
    render: (_, row) => (
      <button
        onClick={() => navigate(`/criteria/${row.id}`)}
        style={{
          border: "none",
  ...
  return (
    <div style={{ color: "#1e293b" }}>
      <PageHeader eyebrow="NAAC RAF STANDARDS" title="NAAC Criteria Overview" description="Manage and track metrics across Criteria 1 through 10." />
      <div className="panel" style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "20px" }}>
        <DataTable columns={columns} data={criteria} keyField="id" emptyMessage="No criteria defined." />
      </div>
    </div>
  );
};
