...
import { institutionApi, type Institution, type InstitutionCreate } from "../api/institutions";
...
export default function Institutions() {
  ...
  const [form, setForm] = useState<InstitutionCreate>({ name: "", type: "", naac_id: "" });
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage registered institutions</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">New Institution</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">NAAC ID (optional)</label>
                <input
                  type="text"
                  value={form.naac_id || ""}
                  onChange={(e) => setForm({ ...form, naac_id: e.target.value || undefined })}
                  className="w-full px-3.5 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  placeholder="NAAC identifier"
                />
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 text-sm">No institutions found</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">NAAC ID</th>
            ...
            <tbody>
              {institutions.map((inst) => (
                <tr key={inst.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{inst.naac_id || "—"}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {new Date(inst.created_at).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 text-xs text-gray-400 font-mono">{inst.id}</td>
                </tr>
              ))}
            </tbody>
          </table>
